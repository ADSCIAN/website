---
index_head:
  text: "Nous soutenons la conception de réponses scientifiquement solides aux défis liés à l'eau, à l'environnement et au climat."
  image_src: "/images/SRE_Landsat_h1600.jpg"

index_intro:
  title: "Votre partenaire en science"
  text: "Nous offrons des services de conseil et de recherche haut de gamme adaptés aux défis environnementaux et climatiques. En tant que data scientists, nous excellons dans l'intégration de données diverses pour analyser et modéliser les systèmes hydriques et environnementaux, fournissant des informations cruciales pour une gestion efficace. Notre réseau académique mondial nous permet de combler le fossé entre le monde universitaire et divers secteurs, garantissant une application rapide des connaissances scientifiques les plus avancées. Nous servons les secteurs publics, les institutions gouvernementales, les entreprises privées, les ONG et fournissons un soutien spécialisé aux universitaires."
  logo_src: "/images/logo_notext.png"

index_services:
  title: "Nos Services"
  subtitle: "Plongez dans des analyses approfondies sur les ressources en eau, la dynamique climatique et les risques associés pour agir en connaissance de cause."
  content:
    - service: "Données & Recherche"
      icon: "/images/svg-icons/linreg-icon.svg"
      description: "Maximisez le potentiel de vos données ou tirez le meilleur parti des ensembles de données disponibles. Grâce à la science des données et à la modélisation, nous renforçons la recherche & développement en offrant une base solide de connaissances pour vos projets et stratégies."
    - service: "Synthèse"
      icon: "images/svg-icons/synthesis-icon.svg"
      description: "Vous avez une question spécifique ? Nous explorons la littérature scientifique, interprétons et synthétisons pour fournir des conclusions innovantes et exploitables adaptées à vos besoins."
    - service: "Formation"
      icon: "/images/svg-icons/training-icon.svg"
      description: "Renforcez votre équipe grâce à des ateliers personnalisés ou des formations dédiées sur l'analyse de données, les SIG, et bien plus encore."
    - service: "Rédaction de propositions"
      icon: "/images/svg-icons/coordination-icon.svg"
      description: "Nous vous aidons pour la rédaction de propositions, d'articles scientifiques et leur révision, en collaboration avec notre société sœur américaine [Bagamian Scientific Consulting](https://www.bagamiansci.com)."

about_us:
  title: "À propos de nous"
  subtitle: "Notre expertise technique couvre les domaines de l’**hydrologie**, des **sciences environnementales** et de l’**analyse de données**."
  text: "Avec des parcours académiques à l'UCLouvain (Belgique) et à l'Université de Floride (USA), nous avons constaté un manque de transfert de l'expertise académique pour relever les défis liés à la gestion des ressources naturelles et au climat. La mission d’ADSCIAN est de combler ce fossé, garantissant que les dernières découvertes académiques bénéficient à un public plus large. En collaborant étroitement avec la communauté académique, nous restons à jour sur les recherches, méthodes et outils les plus récents. Nous nous engageons à l'excellence et à la transparence. Nous promettons à nos clients un partenariat offrant des solutions adaptées à leurs besoins spécifiques."
  cards:
    - name: Alice Alonso, PhD
      desc: Cofondatrice & Associée
      mail: alice.alonso@adscian.be
      gs: https://scholar.google.com/citations?user=IcRj1IIAAAAJ&hl=fr
      in: https://www.linkedin.com/in/alicealonso/
      rg: https://www.researchgate.net/profile/Alice-Alonso
      photo: "/images/aalonso.jpg"
      effect: "fade-up-right"
      duration: 2000
    - name: Damien Delforge, PhD
      desc: Cofondateur & Associé
      mail: damien.delforge@adscian.be
      gs: https://scholar.google.com/citations?user=PFu9yr0AAAAJ&hl=fr
      in: https://www.linkedin.com/in/damien-delforge-10250a59
      rg: https://www.researchgate.net/profile/Damien-Delforge
      photo: "/images/ddelforge.jpg"
      effect: "fade-up-left"
      duration: 2200

adscian_origin:
  question: 'Vous vous demandez ce que signifie <span style="color: #06a6a1;">ADSCIAN</span> ?'
  answer: |
    ADSCIAN est l'acronyme de <strong style="font-weight: bold;">AD
    Scientific consulting and Environmental
    Systems Analytics</strong>.
    AD sont nos initiales communes, et nous l’apprécions car "ad" signifie
    “vers” en latin, ce qui reflète notre mission
    de soutenir nos clients pour un impact plus grand. SCI représente science;
    AN représente analytics, et voilà: ADSCIAN!
---
